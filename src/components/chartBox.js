...
const ChartBox = ({data}) => {
    ...
    for (var i = 1; i < data.patientDatesList.length; i++) {
        ...
        option['options'].push({
            title: {
                'text': 'Relevant mutations from ' + date
            },
            series: [
                {
    ...
    return (
        <div className="chart-box" ref="chart" style={{
            width: 600,
            height: 400
        }}></div>
    );
...
ChartBox.propTypes = {
    data: PropTypes.arrayOf(PropTypes.shape({patientDatesList: PropTypes.array.isRequired, patientSingleLossData: PropTypes.array.isRequired, patientDoubleLossData: PropTypes.array.isRequired, patientGeneExpansionData: PropTypes.array.isRequired}).isRequired).isRequired
}
